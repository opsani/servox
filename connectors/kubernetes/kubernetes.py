...
def update(appname, desc, data, print_progress):
    ...
    for comp_name, comp_data in data.get("application", {}).get("components", {}).items():
        ...
        if dep_name not in raw:
            raise AdjustError('Cannot find deployment with name "{}" for component "{}" in namespace "{}"' \
+                                ''.format(dep_name, comp_name, appname), status="failed", reason="unknown") # FIXME 'reason' code (either bad config or invalid input to update())
        ...
        for name, value in settings.items():
            ...
            cont_patch = patches.setdefault(cont_name, {})
            if name in ("mem", "cpu"):
                set_rsrc(cont_patch, name, value, comp_desc.get('settings', {}).get(name, {}).get('selector', 'both'))
                continue
            elif name == "replicas":
                replicas = int(value)
