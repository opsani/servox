...
class TestMessage:
    ...
    def test_text_message_override_content_type(self) -> None:
        message = servo.pubsub.Message(
            text="A great and insightful message.", content_type="funky/text"
        )
        assert message.text == "A great and insightful message."
    ...
    def test_text_message_raises_if_not_string(self) -> None:
        with pytest.raises(
            ValueError,
            match="Text Messages can only be created with `str` content: got 'int'",
        ):
    ...
    def test_content_message(self) -> None:
        message = servo.pubsub.Message(
            content=b"This is the message", content_type="foo/bar"
        )
...
class TestChannel:
    class TestValidations:
        ...
        def test_exchange_required(self) -> None:
            with pytest.raises(
                TypeError,
                match=re.escape(
                    "__init__() missing 1 required keyword-only argument: 'exchange'"
                ),
            ):
    ...
    async def test_publish(
        self, channel: servo.pubsub.Channel, mocker: pytest_mock.MockFixture
    ) -> None:
    ...
    async def test_publish_fails_if_channel_is_closed(
        self, channel: servo.pubsub.Channel, mocker: pytest_mock.MockFixture
    ) -> None:
        await channel.close()
        assert channel.closed
        with pytest.raises(
            RuntimeError, match="Cannot publish messages to a closed Channel"
        ):
            await channel.publish(servo.pubsub.Message(text="foo"))

    async def test_closing_channel_cancels_exclusive_subscribers(
        self,
        exchange: servo.pubsub.Exchange,
    ...
    async def test_iteration(
        ...
        await task_graph(_publisher(), _subscriber(), timeout=5.0)
        assert messages
...
class TestSubscription:
    ...
    def test_match_by_glob(
        self, exchange: servo.pubsub.Exchange, selector: str, matches: bool
    ) -> None:
        metrics_channel = servo.pubsub.Channel(
            name="metrics.prometheus.http", exchange=exchange
        )
        message = servo.pubsub.Message(text="foo")
    ...
    def test_match_by_regex(
        ...
        subscription = servo.pubsub.Subscription(selector=selector)
        assert (
            subscription.matches(channel, message) == matches
        ), f"expected regex pattern '{selector}' match of '{channel.name}' to == {matches}"


@pytest.fixture
def subscriber(
    exchange: servo.pubsub.Exchange, subscription: servo.pubsub.Subscription
) -> servo.pubsub.Subscriber:
...
class TestSubscriber:
    ...
    async def test_sync_callback_is_invoked(
        self, subscriber: servo.pubsub.Subscriber, mocker: pytest_mock.MockerFixture
    ) -> None:
    ...
    async def test_async_callback_is_invoked(
        self, subscriber: servo.pubsub.Subscriber, mocker: pytest_mock.MockerFixture
    ) -> None:
    ...
    async def iterator_task_factory(self) -> Callable[[], asyncio.Task]:
        ...
        async def _iterator_task_factory(
            subscriber: servo.pubsub.Subscriber,
        ) -> asyncio.Task:
            event = asyncio.Event()
    ...
    async def test_multiple_iterators(
        self,
        subscriber: servo.pubsub.Subscriber,
        iterator_task_factory: Callable[[], asyncio.Task],
    ) -> None:
        message = servo.pubsub.Message(text="foo")
        channel = servo.pubsub.Channel(name="metrics", exchange=subscriber.exchange)
    ...
    async def test_iterator_context(
        self, channel: servo.pubsub.Subscriber, subscriber: servo.pubsub.Subscriber
    ) -> None:
        other_subscriber = servo.pubsub.Subscriber(
            exchange=subscriber.exchange, subscription=subscriber.subscription
        )
    ...
    async def test_cannot_stop_inactive_iterator(
        self, channel, subscriber: servo.pubsub.Subscriber
    ) -> None:
        other_subscriber = servo.pubsub.Subscriber(
            exchange=subscriber.exchange, subscription=subscriber.subscription
        )
        ...
        async def _test() -> self:
            with pytest.raises(
                RuntimeError, match="Attempted to stop an inactive iterator"
            ):
                async for message_, channel_ in subscriber:
                    iterator = servo.pubsub._current_iterator()
                    assert iterator
        ...
        await asyncio.gather(
            _test(), subscriber(servo.pubsub.Message(text="foo"), channel), event.wait()
        )

    async def test_cannot_stop_without_an_iterator(
        self, subscriber: servo.pubsub.Subscriber
    ) -> None:
        with pytest.raises(
            RuntimeError, match="Attempted to stop outside of an iterator"
        ):
            subscriber.stop()

    async def test_cancellation_stops_all_iterators(
        self, channel, subscriber: servo.pubsub.Subscriber
    ) -> None:
        ...
        await asyncio.wait_for(
            task_graph(
                _cancel(), _create_iterator(), _create_iterator(), _create_iterator()
            ),
            timeout=1.0,
        )

    async def test_cannot_stop_nested_iterator(
        self,
        subscriber: servo.pubsub.Subscriber,
        iterator_task_factory: Callable[[], asyncio.Task],
    ) -> None:
...
@pytest.fixture
def publisher(
    exchange: servo.pubsub.Exchange, channel: servo.pubsub.Channel
) -> servo.pubsub.Publisher:
...
def multipublisher(exchange: servo.pubsub.Exchange) -> servo.pubsub.Publisher:
    ...
    normalized_metrics = servo.pubsub.Channel(
        name="metrics.normalized", exchange=exchange
    )
    return servo.pubsub.Publisher(
        exchange=exchange, channels=[raw_metrics, normalized_metrics]
    )


class TestPublisher:
    async def test_calling_publishes_to_exchange(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
        message = servo.pubsub.Message(text="foo")
    ...
    async def test_calling_with_one_channel(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
        message = servo.pubsub.Message(text="foo")
        with servo.utilities.pydantic.extra(multipublisher.exchange):
    ...
    async def test_calling_with_two_channels(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
        message = servo.pubsub.Message(text="foo")
        with servo.utilities.pydantic.extra(multipublisher.exchange):
    ...
    async def test_calling_with_one_channel_name(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
        message = servo.pubsub.Message(text="foo")
        with servo.utilities.pydantic.extra(multipublisher.exchange):
    ...
    async def test_calling_with_invalid_channel(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
        message = servo.pubsub.Message(text="foo")
        with servo.utilities.pydantic.extra(multipublisher.exchange):
            spy = mocker.spy(multipublisher.exchange, "publish")
            with pytest.raises(
                ValueError, match="Publisher is not bound to Channel: 'invalid'"
            ):
                await multipublisher(message, "invalid")

    async def test_switching_message_by_channel(
        self, multipublisher: servo.pubsub.Publisher, mocker: pytest_mock.MockFixture
    ) -> None:
...
class TestFilter:
    async def test_passthrough_filter(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
    ...
    async def test_cancel_with_async_callback(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        filter_event = asyncio.Event()
    ...
    async def test_cancel_with_sync_callback(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        filter_event = asyncio.Event()
    ...
    async def test_modify_message_async_callback(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        filter_event = asyncio.Event()

        async def _uppercase_message(
            message: servo.pubsub.Message, channel: servo.pubsub.Channel
        ) -> servo.pubsub.Message:
            filter_event.set()
    ...
    async def test_modify_message_sync_callback(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        filter_event = asyncio.Event()

        def _uppercase_message(
            message: servo.pubsub.Message, channel: servo.pubsub.Channel
        ) -> servo.pubsub.Message:
            filter_event.set()
            return servo.pubsub.Message(text=message.text.upper())
...
class TestSplitter:
    async def test_mirror_to_channels(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        ...
        async def _mirror_message(
            splitter: servo.pubsub.Splitter,
            message: servo.pubsub.Message,
            channel: servo.pubsub.Channel,
        ) -> None:
        ...
        splitter = servo.pubsub.Splitter(
            _mirror_message, one_channel, two_channel, three_channel, remainder_channel
        )
        ...
        calls = [
            mocker.call(message, metrics_channel),
    ...
    async def test_split_across_channels(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        metrics_channel = exchange.create_channel("metrics")
        one_channel = exchange.create_channel("one")
        ...
        async def _split_message(
            ...
            if channel == metrics_channel:
                ...
                words_to_channel = dict(
                    zip(
                        words,
                        [one_channel, two_channel, three_channel, remainder_channel],
                    )
                )
        ...
        splitter = servo.pubsub.Splitter(
            _split_message, one_channel, two_channel, three_channel, remainder_channel
        )
        exchange.add_transformer(splitter)
...
class TestAggregator:
    async def test_repr(
        ...
        assert (
            repr(aggregator)
            == "Aggregator(from_channels=['prometheus', 'cloudwatch'], to_channel='scaling', every=None, message=None, last_published_at=None, channel_message_count={'prometheus': 0, 'cloudwatch': 0})"
        )

    async def test_aggregating_two_channels(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        prometheus_metrics = exchange.create_channel("prometheus")
        cloudwatch_metrics = exchange.create_channel("cloudwatch")
        ...
        exchange.start()
        prometheus_message = servo.pubsub.Message(
            text="This message is from Prometheus"
        )
        cloudwatch_message = servo.pubsub.Message(
            text="This message is from Cloudwatch"
        )
    ...
    async def test_timer_fires_repeatedly(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        # Don't publish a message for the second channel and let it fire with current state
        prometheus_metrics = exchange.create_channel("prometheus")
...
class TestExchange:
    ...
    async def test_create_channel_names_must_be_unique(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
        exchange.create_channel("whatever")
        with pytest.raises(
            ValueError, match="A Channel named 'whatever' already exists"
        ):
            exchange.create_channel("whatever")
    ...
    async def test_publish_to_channel_by_name(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        exchange.start()
        channel = exchange.create_channel("metrics")
    ...
    async def test_publish_to_unknown_channel_fails(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
        message = servo.pubsub.Message(text="Testing")
    ...
    async def test_publish_when_not_running_enqueues(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
        channel = exchange.create_channel("metrics")
        message = servo.pubsub.Message(text="Testing")
    ...
    async def test_create_publisher_by_channel_name(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
        channel = exchange.create_channel("metrics")
    ...
    async def test_create_publisher_creates_channels(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
    ...
    async def test_create_subscriber_with_timeout(
        self, exchange: servo.pubsub.Exchange
    ) -> None:
        exchange.start()
    ...
    async def test_repr(self, exchange: servo.pubsub.Exchange) -> None:
        ...
        assert (
            repr(exchange)
            == "Exchange(running=False, channel_names=['whatever'], publisher_count=1, subscriber_count=1, transformer_count=0, queue_size=0)"
        )
    ...
    async def test_current_message_in_callback(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        exchange.start()
    ...
    async def test_current_message_in_iterator(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockerFixture
    ) -> None:
        exchange.start()
        publisher = exchange.create_publisher("metrics.http.production")
    ...
    async def test_iteration(
        self, exchange: servo.pubsub.Exchange, mocker: pytest_mock.MockFixture
    ) -> None:
        channel = exchange.create_channel("whatever")
        messages = []
        ...
        async def _publisher() -> None:
            for i in range(3):
                await exchange.publish(
                    servo.pubsub.Message(text=f"Message: {i}"), channel
                )
...
class HostObject(servo.pubsub.Mixin):
    ...
    async def _test_subscriber_decorator(
...
class TestMixin:
    ...
    async def test_exchange_property_setter(
        self, host_object: HostObject, exchange: servo.pubsub.Exchange
    ) -> None:
        assert host_object.pubsub_exchange
    ...
    async def test_publisher_context_manager_rejects_every_arg(
        self, host_object: HostObject
    ) -> None:
        with pytest.raises(
            TypeError,
            match="Cannot create repeating publisher when used as a context manager: `every` must be None",
        ):
    ...
    async def test_publisher_callable_await(self, host_object: HostObject) -> None:
        await host_object.publish(
            servo.pubsub.Message(
                json={"whatever": ["you", "want", "if", "json", "serializable"]}
            ),
            "metrics",
        )
        assert len(host_object.pubsub_exchange._publishers) == 0
    ...
    async def test_subscriber_decorator(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
        event = asyncio.Event()
        stub = mocker.stub()
    ...
    async def test_subscriber_context_manager(
        ...
        await asyncio.wait_for(asyncio.gather(_publisher(), _subscriber()), timeout=3.0)
        stub.assert_called_once_with(message, channel)

    async def test_pubsub_between_decorators(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
        notifications = []
    ...
    async def test_cancel_subscribers(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
        stub = mocker.stub()
        await host_object._test_subscriber_decorator(stub)
    ...
    async def test_cancel_all_subscribers(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
        stub = mocker.stub()
    ...
    async def test_cancel_publishers(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
        await host_object._test_publisher_decorator()
    ...
    async def test_cancel_all_publishers(
        self, host_object: HostObject, mocker: pytest_mock.MockFixture
    ) -> None:
    ...
    async def test_channel_async_context_manager_temporary(
        self, host_object: HostObject
    ) -> None:
    ...
    async def test_channel_async_context_manager_existing(
        self, host_object: HostObject
    ) -> None:
    ...
    async def test_channel_async_context_manager_temporary_random_name(
        self, host_object: HostObject
    ) -> None:
        name = None
        channel_ref = None
    ...
    async def test_exception_handling_in_publisher(
        self, host_object: HostObject
    ) -> None:
...
async def task_graph(task, *dependencies, timeout: Optional[servo.Duration] = None):
    ...
    await asyncio.wait_for(
        asyncio.gather(_main_task(), *list(map(_dependent_task, dependencies))),
        timeout=timeout_,
    )
