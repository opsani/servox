...
class TestSetting:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Failures
            (True, 1, None, 'value of pinned settings cannot be changed: assigned value None is not equal to existing value 1'),
            ...
            (True, 5.0, 2.5, 'value of pinned settings cannot be changed: assigned value 2.5 is not equal to existing value 5.0'),
            (True, 5.0, 3, 'value of pinned settings cannot be changed: assigned value 3 is not equal to existing value 5.0'),
            (True, 1, "1", "value of pinned settings cannot be changed: assigned value '1' is not equal to existing value 1"),
            (True, "something", "another", "value of pinned settings cannot be changed: assigned value 'another' is not equal to existing value 'something'"),
            (True, "something", 5.0, "value of pinned settings cannot be changed: assigned value 5.0 is not equal to existing value 'something'"),

            # Coerced types
            (True, 5.0, 5, None),
            (True, 5, 5.0, None),
            (True, "5", 5.0, "value of pinned settings cannot be changed: assigned value 5.0 is not equal to existing value '5'"),
        ]
    )
    def test_validate_pinned_value_cannot_change(self, 
        pinned: bool, init_value: Union[str, Numeric], new_value: Union[str, Numeric], error_message: str
    ) -> None:
...
class TestEnumSetting:
    ...
    def setting(self) -> EnumSetting:
        return EnumSetting(unit="unit", name="the-setting", values=["one", "two", "three"], value="two")
    ...
    def test_unit_value_are_excluded_from_opsani_repr_if_none(self) -> None:
        setting = EnumSetting(unit=None, name="the-setting", values=["one", "two", "three"], value=None)
        assert setting.__opsani_repr__() == {
            'the-setting': {'pinned': False,
                            'type': 'enum',
                            'values': ['one',
    ...
    def test_validate_values_list_is_not_empty(self) -> None:
        ...
        assert error.value.errors()[0]["msg"] == "ensure this value has at least 1 items"
    ...
    def test_validate_value_is_included_in_values_list(self) -> None:
        ...
        assert error.value.errors()[0]["msg"] == "invalid value: 'three' is not in the values list ['one', 'two']"
...
class TestRangeSetting:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (1, 2, 3, None),
            (1.0, 2.0, 3.0, None),
            (1.0, 2, 3, "invalid range: min, max, and step must all be of the same Numeric type (float: min. int: max, step.)"),
            (1, 2.0, 3, "invalid range: min, max, and step must all be of the same Numeric type (int: min, step. float: max.)"),
            (1, 2, 3.0, "invalid range: min, max, and step must all be of the same Numeric type (int: min, max. float: step.)"),
        ]
    )
    def test_validate_all_elements_of_range_are_same_type(self, 
        min: Numeric, max: Numeric, step: Numeric, error_message: str
    ) -> None:
        if error_message is not None:
            with pytest.raises(pydantic.ValidationError) as error:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (1.0, 5.0, 1.0, 10.0, "invalid value: 10.0 is outside of the range 1.0-5.0"),
            (1.0, 10.0, 1.0, 0.0, "invalid value: 0.0 is outside of the range 1.0-10.0"),
        ]
    )
    def test_value_falls_in_range(self,
        min: Numeric, max: Numeric, step: Numeric, value: Numeric, error_message: str
    ) -> None:
        if error_message is not None:
            with pytest.raises(pydantic.ValidationError) as error:
    ...
    def test_warning_if_not_multiple_of_step(self) -> None:
        ...
        try:
            ...
            assert "RangeSetting('misaligned' 0.0-25.0, 5.0) value is not step aligned: 16.0 is not divisible by 5.0" in messages[0]
        finally:
            reset_to_defaults()
...
class TestReplicas:
    ...
    def test_range_fields_strict_integers(self, field_name: str, required: bool, allow_none: bool) -> None:
    ...
    def test_validate_name_cannot_be_changed(self) -> None:
        ...
        assert error.value.errors()[0]["loc"] == ("name",)
        assert error.value.errors()[0]["type"] == "value_error.const"
        assert error.value.errors()[0]["msg"] == "unexpected value; permitted: 'replicas'"
...
class TestInstanceType:
    ...
    def test_validate_name_cannot_be_changed(self) -> None:
        with pytest.raises(pydantic.ValidationError) as error:
            InstanceType(name="other", values=["this", "that"], unit=InstanceTypeUnits.EC2)
    
        assert error
        assert "1 validation error for InstanceType" in str(error.value)
        assert error.value.errors()[0]["loc"] == ("name",)
        assert error.value.errors()[0]["type"] == "value_error.const"
        assert error.value.errors()[0]["msg"] == "unexpected value; permitted: 'inst_type'"
