import pytest
from servo.connectors.prometheus import PrometheusConfiguration, PrometheusConnector, PrometheusMetric
...
class TestPrometheusConfiguration:
    ...
    def test_base_url_is_rstripped(self):
        config = PrometheusConfiguration(base_url="http://prometheus.io/some/path/", metrics=[])
    ...
    def test_supports_cluster_url(self):
        config = PrometheusConfiguration(base_url="http://prometheus.default.svc.cluster.local:9090", metrics=[])
    ...
    def test_api_url(self):
        config = PrometheusConfiguration(base_url="http://prometheus.default.svc.cluster.local:9090", metrics=[])
        assert config.api_url == "http://prometheus.default.svc.cluster.local:9090/api/v1/"
