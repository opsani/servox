...
async def minikube_servo_image(minikube: str, servo_image: str, subprocess) -> str:
    ...
    exit_code, _, _ = await subprocess(
        f"minikube cache add -p {minikube} {servo_image}", print_output=True
    )
    if exit_code != 0:
        raise RuntimeError(
...
async def kind(request, subprocess, kubeconfig: str, kubecontext: str) -> str:
    ...
    if marker:
        assert (
            len(marker.args) == 1
        ), f"kind_cluster marker accepts a single argument but received: {repr(marker.args)}"
    ...
    exit_code, _, _ = await subprocess(
        f"kind get clusters | grep {cluster} || kind create cluster --name {cluster} --kubeconfig {kubeconfig}",
        print_output=True,
    )
    if exit_code != 0:
    ...
    try:
        ...
        await subprocess(
            f"kubectl config --kubeconfig {kubeconfig} use-context {kubecontext}",
            print_output=True,
        )
        ...
        if not os.getenv("GITHUB_ACTIONS"):
            exit_code, _, _ = await subprocess(
                f"kind delete cluster --name {cluster} --kubeconfig {kubeconfig}",
                print_output=True,
            )
            if exit_code != 0:
                raise RuntimeError(
                    f"failed running kind delete: exited with status code {exit_code}"
                )
...
@pytest.fixture
async def kind_servo_image(
    kind: str, servo_image: str, subprocess, kubeconfig: str
) -> str:
    ...
    exit_code, _, _ = await subprocess(
        f"kind load docker-image --name {kind} {servo_image}", print_output=True
    )
    if exit_code != 0:
        raise RuntimeError(f"failed running kind: exited with status code {exit_code}")
