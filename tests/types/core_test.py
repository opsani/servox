...
class TestDurationProgress:
    ...
    async def test_elapsed_is_none_when_not_started(self, progress: DurationProgress) -> None:
        assert not progress.started
        assert progress.elapsed is None

    async def test_elapsed_is_duration_when_started(self, progress: DurationProgress) -> None:
        assert not progress.started
        assert progress.elapsed is None
        progress.start()
        assert isinstance(progress.elapsed, Duration)

    async def test_progress_is_zero_when_not_started(self, progress: DurationProgress) -> None:
        assert not progress.started
        assert progress.progress == 0.0

    async def test_progress_is_float_when_started(self, progress: DurationProgress) -> None:
        assert not progress.started
        assert progress.elapsed is None
        progress.start()
        assert isinstance(progress.progress, float)

    async def test_async_iterator_updates(self, progress: DurationProgress, mocker: pytest_mock.MockFixture) -> None:
        stub = mocker.stub()
