...
async def test_run_subprocess_exec():
    status_code, stdout, stderr = await servo.utilities.subprocess.run_subprocess_exec(
        "/bin/echo", "test"
    )
    assert status_code == 0
    assert stdout == ["test"]
...
async def test_run_subprocess_shell():
    status_code, stdout, stderr = await servo.utilities.subprocess.run_subprocess_shell(
        "echo test"
    )
    assert status_code == 0
    assert stdout == ["test"]
