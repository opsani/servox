...
async def test_stream_subprocess_exec():
    output = []
    status_code = await stream_subprocess_exec("/bin/echo", "test", stdout_callback=lambda m: output.append(m))
...
async def test_stream_subprocess_shell():
    output = []
    status_code = await stream_subprocess_shell("cd ~/ && echo test", stdout_callback=lambda m: output.append(m))
    assert status_code == 0
    assert output == ["test"]
...
async def test_run_subprocess_timeout():
    output = []
    await stream_subprocess_shell("echo 'test'", stdout_callback=lambda m: output.append(m), timeout=10.0)
    assert output == ['test']
