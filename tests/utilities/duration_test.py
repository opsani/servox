...
import pytest
from servo.utilities import timedelta_from_duration_str, timedelta_to_duration_str, DurationError
...
def test_parser():
    for [input, passes, expected] in cases:
        if passes:
            ...
            assert expected == actual, "{}, expecting {}, got {}".format(input, expected, actual)
        else:
