...
from servo.types import DataPoint, Metric, Reading, SloCondition, SloInput, SloKeep, TimeSeries, Unit
...
def test_non_unique_conditions() -> None:
    ...
    assert str(err_info.value) == ("1 validation error for SloInput\n"
        "conditions\n"
...
def test_trigget_count_greter_than_window() -> None:
    ...
    assert str(err_info.value) == ("1 validation error for SloCondition\n"
    "__root__\n"
...
def _make_time_series_list(metric: Metric, values: List[List[float]]) -> List[TimeSeries]:
    ret_list = []
