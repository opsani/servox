...
def assert_equal_callable_descriptors(
    *descriptors: Tuple[CallableDescriptor, ...], 
    ...
    reference_parameters: typing.Mapping[str, inspect.Parameter] = reference_descriptor.signature.parameters
    ...
    for descriptor in descriptors[1:]:
        descriptor_parameters: typing.Mapping[str, inspect.Parameter] = descriptor.signature.parameters
        ...
        if descriptor.signature.return_annotation != reference_descriptor.signature.return_annotation:
        ...
        extraneous_keywords = sorted(
            list(set(descriptor_keyword_nonvar.keys()) - set(reference_keyword_nonvar.keys()))
        )
        ...
        for index, (parameter_name, reference_parameter) in enumerate(reference_parameters.items()):
            ...
            if reference_parameter.kind == inspect.Parameter.POSITIONAL_ONLY:
                if index > len(descriptor_positional_parameters) - 1:
                    if descriptor_positional_parameters[-1].kind != inspect.Parameter.VAR_POSITIONAL:
                        raise TypeError(
                ...
                if descriptor_parameter != inspect.Parameter.VAR_POSITIONAL:
                    if descriptor_parameter.annotation != reference_parameter.annotation:
                        ...
                        assert_equal_types(reference_parameter_type, descriptor_parameter_type)
                ...
                assert reference_parameter.kind == inspect.Parameter.VAR_KEYWORD, reference_parameter.kind
