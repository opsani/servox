...
class SloCondition(BaseModel):
    ...
    def _check_threshold_values(cls, values):
        if values.get('threshold') is not None and values.get('threshold_metric') is not None:
            raise ValueError("SLO Condition cannot specify both threshold and threshold_metric")

        if values.get('threshold') is None and values.get('threshold_metric') is None:
            raise ValueError("SLO Condition must specify either threshold or threshold_metric")
    ...
    def _trigger_count_cannot_be_greater_than_window(cls, values) -> Numeric:
        trigger_window, trigger_count = values["trigger_window"], values["trigger_count"]
        if trigger_count > trigger_window:
            raise ValueError(f"trigger_count cannot be greater than trigger_window ({trigger_count} > {trigger_window})")
