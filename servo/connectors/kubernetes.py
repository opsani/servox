...
class HPA(KubernetesModel):
    ...
    #
    #     Returns:
    #         The status of the HPA.
    #     """
    #     # first, refresh the hpa state to ensure latest status
    #     await self.refresh()
    #
    #     # return the status of the hpa
    #     breakpoint()
    #     return cast(kubernetes_asyncio.client.V1PodStatus, self.obj.status)

    @property
    def cpu_utilization_scaling_threshold(self) -> float:
        breakpoint()
        # return list(map(lambda c: Container(c, self), self.obj.spec.containers))
        a=1

    async def get_cpu_utilization_scaling_threshold(self) -> float:
        await self.refresh()
        breakpoint()
        return self.cpu_utilization_scaling_threshold
        a=1

    def uid(self) -> str:
        """
        Gets the UID for the HPA.
        ...
        """
        # TODO edit docs, idk if this statement ^ holds for HPAs.
        breakpoint()
...
class KubernetesOptimizations(pydantic.BaseModel, servo.logging.Mixin):
    ...
    def find_optimization(self, name: str) -> Optional[BaseOptimization]:
        ...
        """
        breakpoint()  # what's self.optimizations?
...
class KubernetesConnector(servo.BaseConnector):
    ...
    async def _measure_hpa_cpu_utilization_threshold(self) -> Optional[float]:
    ...
    async def measure(
        ...
        if self.config.hpa:
            ...
            future = asyncio.create_task(self._measure_hpa_cpu_utilization_threshold())
