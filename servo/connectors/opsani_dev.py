...
class OpsaniDevChecks(checks.BaseChecks):
    ...
    async def check_kubernetes_service_type(self) -> None:
        ...
        if not service.spec.type in ("ClusterIP", "LoadBalancer"):
            raise ValueError(f"expected service type of ClusterIP or LoadBalancer but found {service.spec.type}")
    ...
    async def _read_servo_pod(self) -> Optional[kubernetes.Pod]:
        return (
            await self._read_servo_pod_from_env()
            or next(reversed(await self._list_servo_pods()), None)
        )
    ...
    async def _list_servo_pods(self) -> List[kubernetes.Pod]:
        ...
        async with kubernetes.Pod.preferred_client() as api_client:
            label_selector = kubernetes.selector_string({ "app.kubernetes.io/name": "servo" })
            pod_list: kubernetes.client.V1PodList = await api_client.list_namespaced_pod(
